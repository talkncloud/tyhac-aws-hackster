...
tableIndex = os.environ.get('DYNAMO_INDEX')
...
def handler(event, context):
    ...
    try:
        ...
        # updateMqtt(bucketKey, "fail") # no longer required at this point, monitor
        return
...
def predictCough(x, fs, localFile, bucketKey):
    ...
    try:
        ...
        # I found that confidence for items other than coughs was good i.e low score, but the coughs
        # in the raw data we have are tricky to pass through this check. They would register low
        # scores 15, 20, 40, 60 % tops. This led to this first check being quite low. 
...
def downloadFile(bucketKey):
    ...
    try:
        ...
        # This has likely happend on retry, we need to update the client
...
def getRecord(recordId):
    ...
    try:
        ...
        response = table.query(
            ...
            KeyConditionExpression=Key('filename').eq(uid)
        )

        logger.info("TYHAC: recordId - " + recordId + " - " + str(response['Items']))

        return json.dumps(response['Items'][0], cls=DecimalEncoder)

    except Exception as e:
        logger.error("TYHAC: error getting dynamodb record - " + str(e))
        # TODO: there appears to be an issue where the record is not found in dynamodb, attempts to leverage indexes, con reads etc
        #       have not helped. this condition will cause the lambda function to retry to update the client properly. Needs
        #       review.
        exit(1) # non-standard, retry the function
